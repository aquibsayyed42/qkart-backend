...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should throw 404 error if cart is not present", async () => {
       ...
       * }
       */
      // return expect(res.statusCode).rejects.toEqual(404)
      return expect(res).rejects.toEqual(new ApiError(404,"User does not have a cart"))
    ...
    it("should throw 400 error if user's cart doesn't have any product", async () => {
      ...
      // expect(res.statusCode).rejects.toEqual(400)
    ...
    it("should throw 400 error if wallet balance is insufficient", async () => {
      ...
      await expect(res).rejects.toThrow(
        expect.objectContaining({
          statusCode: 400,
          message: "User has insufficient money to process",
        })
      );
    });
